  ...
  if(history.length > 1) {
    ...
    for(let i = 0; i < size; i++) {
      ...
      for(let j = 0; j < size; j++) {
        row.innerHTML += `
          <button id="column${j + 1} row${i + 1}" class="game-btn size-${size} ${prevState[numLoops] === "1" ? "on" : "off"}">
          </button>
        `
        numLoops++
      }
...
const renderGame = (size, gameMode) => {
  ...
  for(let i = 0; i < size; i++) {
    ...
    const row = document.querySelector(`#row${i + 1}`)
    for(let j = 0; j < size; j++) {
      state = gameMode === "classic" ? Math.random() < chance ? "on" : "off" : "off"
      row.innerHTML += `
        <button id="column${j + 1} row${i + 1}" class="game-btn size-${size} ${state}">
        </button>
      `
    }
  }
  ...
  undo.addEventListener("click", () => {undoHandler(size)})

  const reset = document.querySelector("#reset")
  reset.addEventListener("click", () => {resetHandler(size)})
}
...
const matchBtnHandler = (e) => {
  ...
  for(let i = 0; i < size; i++) {
    ...
    for(let j = 0; j < size; j++) {
      row.innerHTML += `
        <div id="match column${j + 1} row${i + 1}" class="match-btn match-size-${size} ${shapeArr[numLoops] === "1" ? "on" : "off"}">
        </div>
      `
      numLoops++
    }
